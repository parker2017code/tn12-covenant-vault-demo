...
fn spk_script_hex(spk: &kaspa_consensus_core::tx::ScriptPublicKey) -> String {
    bytes_to_hex(spk.script().as_ref())
}

fn load_source(repo_root: &str, name: &str) -> String {
...
fn state(mux_template: &[u8], a_template: &[u8], b_template: &[u8], value: i64, timeout: i64, pending: i64) -> Vec<Expr<'static>> {
    vec![
        bytes_expr(mux_template),
        bytes_expr(a_template),
        bytes_expr(b_template),
        Expr::int(value),
        Expr::int(timeout),
        Expr::int(pending),
    ]
...
fn main() {
    ...
    let input_value = std::env::var("INPUT_VALUE_SOMPI").unwrap_or_else(|_| "1000000000".to_string()).parse::<u64>().unwrap();
    ...
    let input_contract = compile_member(&mux_source, &mux_template, &a_template, &b_template, 5, 10, 0);
    let output_contract = compile_member(&a_source, &mux_template, &a_template, &b_template, 5, 10, 1);
    ...
    println!("{}", serde_json::to_string_pretty(&json!({
        ...
        "templates": {
            "mux": bytes_to_hex(&mux_template),
            "a": bytes_to_hex(&a_template),
            "b": bytes_to_hex(&b_template),
        ...
        "state": { "value": 5, "timeout": 10, "prevPending": 0, "nextPending": 1, "selectedWorker": "A", "minerFeeSompi": fee },
