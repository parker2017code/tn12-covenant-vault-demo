...
fn main() {
    ...
    let input_value = std::env::var("INPUT_VALUE_SOMPI").unwrap_or_else(|_| "999940000".to_string()).parse::<u64>().unwrap();
    ...
    let value = std::env::var("VALUE").unwrap_or_else(|_| "8".to_string()).parse::<i64>().unwrap();
    let timeout = std::env::var("TIMEOUT").unwrap_or_else(|_| "10".to_string()).parse::<i64>().unwrap();
    let sequence = std::env::var("SEQUENCE").unwrap_or_else(|_| "10".to_string()).parse::<u64>().unwrap();

    let input_contract = compile_member(&a_source, &mux_template, &a_template, &b_template, value, timeout, 1);
    let output_contract = compile_member(&mux_source, &mux_template, &a_template, &b_template, value - 1, timeout, 0);
    let covenant_id = Hash::from_bytes(hex_to_array_32(&covenant_id_hex));
    let sigscript = p2sh_sigscript(&input_contract, "timeout", vec![bytes_expr(&mux_prefix), bytes_expr(&mux_suffix)]);
    ...
    println!("{}", serde_json::to_string_pretty(&json!({
        ...
        "state": { "valueBefore": value, "valueAfter": value - 1, "timeout": timeout, "sequence": sequence, "prevPending": 1, "nextPending": 0, "minerFeeSompi": fee },
