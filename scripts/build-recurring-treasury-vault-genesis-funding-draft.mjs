...
const sourcePath = process.env.RECURRING_TREASURY_GENESIS_SOURCE || "fixtures/FundedWalletOutpoint.json";
const walletPath = process.env.TN12_WALLET || ".local/tn12-wallet.json";
const contractPath = process.env.RECURRING_TREASURY_CONTRACT || "artifacts/RecurringTreasuryVault.json";
const outPath = process.env.OUT || "artifacts/signed-drafts/recurring-treasury-vault-genesis-funding.json";
const amountTkas = process.env.RECURRING_TREASURY_FUNDING_TKAS || "150";
...
const output0 = signed.outputs[0].toJSON();
...
const artifact = {
  schema: "tn12-recurring-treasury-vault-genesis-funding-draft/v1",
  network: "kaspa-testnet-12",
  status: covenant ? "signed-covenant-genesis-not-broadcast" : "blocked-covenant-binding-missing",
  warning: "This is a signed TN12 draft. It has not been submitted. Submit only after checking the source UTXO is still unspent.",
  ...
  nextAfterAccepted: [
    ...
    "Fetch the accepted tx and record output 0 as the new RecurringTreasuryVault contract outpoint.",
    "Rerun the RPC data route; the funding output must expose a covenant binding.",
    "Only then attempt the recurring-vault under-cap spend."
  ]
};
