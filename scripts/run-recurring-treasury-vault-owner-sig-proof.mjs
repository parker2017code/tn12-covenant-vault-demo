...
const rustSource = "scripts/rust/recurring_treasury_vault_owner_sig_proof.rs";
const outPath = process.env.OUT || "artifacts/recurring-treasury-vault-owner-sig-proof.json";
...
const tmp = await mkdtemp(join(tmpdir(), "tn12-recurring-vault-owner-sig-"));
try {
  ...
  const artifact = {
    schema: "tn12-recurring-treasury-vault-owner-sig-proof/v1",
    reviewedAt: "2026-05-12",
    status: run.code === 0 ? "local-owner-sig-covenant-proof-passed" : "local-owner-sig-covenant-proof-failed",
    source: "contracts/RecurringTreasuryVault.sil",
    ...
    cases,
    proves: [
      "owner signature is checked on the full RecurringTreasuryVault.sil path",
      "under-cap spend can pay the required destination and relock continuation state",
...
function cargoToml(root) {
  return `[package]
name = "tn12-recurring-vault-owner-sig-proof"
version = "0.1.0"
edition = "2024"
