...
assert.equal(coordinationSettlementBrief.blockedPacks.length, 1);
assert.equal(coordinationSettlementBrief.blockedPacks[0].packId, "pack-stag-liquidity-migration-research");
assert.equal(coordinationSettlementBrief.blockedPacks[0].nextRoute, "refund-or-keep-accumulating");
...
assert.ok(projectPlan.done.some((item) => item.id === "based-app-prototypes"));
...
assert.ok(projectPlan.next.some((item) => item.id === "based-app-vertical"));
...
assert.match(readme, /Current Gaps/i);
...
assert.match(auditMap, /SCRIPT_ENFORCED/);
assert.match(auditMap, /INDEXER_DERIVED/);
assert.match(auditMap, /operator-receipt-pack\.json/);
assert.match(auditMap, /npm run operator:refresh/);
...
for (const [name, content] of publicHtml) {
  ...
  assert.doesNotMatch(content, /Active build plan|Artifact work order|Status map|Mainnet readiness map|Proven status|Operator pack|Next 10|Evidence map|Adapter backlog/i, `${name} exposes internal status/planning labels`);
  assert.doesNotMatch(content, /\b\d{1,3}%\b.*\b(complete|ready|readiness|benchmark|done)\b/i, `${name} exposes score-style completion language`);
  assert.doesNotMatch(content, /completion estimate|readiness map|mainnet readiness|full-DeFi benchmark|repo-local benchmark/i, `${name} exposes docs-only completion/readiness labels`);
  assert.doesNotMatch(content, /external signer|external signing|external-wallet|external wallet/i, `${name} should say user-wallet signing in public copy`);
}
...
assert.match(html, /Open the deeper workbench/);
assert.doesNotMatch(html, /Assurance contract/);
assert.doesNotMatch(html, /Miner signal research/);
assert.match(html, /Product maps, wallet drafts, pool-style replay checks, scheduler work, and generated artifact inventories live in Lab Tools/);
assert.doesNotMatch(html, /Enforcement matrix/);
assert.doesNotMatch(html, /Mainnet readiness map/);
assert.doesNotMatch(html, /Artifact map/);
assert.match(html, /receipt alone is not custody settlement/i);
assert.doesNotMatch(html, /receipt-events/);
...
assert.match(labHtml, /App receipt path/);
...
assert.match(html, /wallet drafts, pool-style replay checks, scheduler work/);
...
assert.match(labHtml, /Access passes/);
...
assert.match(labHtml, /What still needs proof/);
assert.doesNotMatch(labHtml, /Active build plan/);
assert.doesNotMatch(labHtml, /Artifact work order/);
assert.doesNotMatch(labHtml, /npm run project:plan/);
assert.doesNotMatch(labHtml, /npm run project:queue/);
assert.match(labHtml, /Reference library/);
assert.match(labHtml, /App map/);
assert.match(labHtml, /Attestation registry/);
assert.match(labHtml, /Event signal simulator/);
assert.match(labHtml, /npm run prediction:hedge/);
assert.match(labHtml, /escrow mutual-cancel proof transactions/);
assert.match(labHtml, /user-wallet signing, live rollback evidence, and one concrete settlement path/);
