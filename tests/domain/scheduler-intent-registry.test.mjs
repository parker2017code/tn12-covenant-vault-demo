...
assert.equal(registry.summary.blockedNegativeRows, 7);
assert.equal(registry.summary.executionReceipts, 1);
assert.equal(registry.summary.schedulerBids, 3);
assert.equal(registry.summary.winningBids, 1);
assert.equal(registry.summary.blockedAuctionRows, 2);
...
assert.equal(registry.auctionRows.find((row) => row.status === "winner-selected")?.bidder, "local-pool-operator");
assert.ok(registry.auctionRows.every((row) => row.enforcement === "PLANNER_ONLY"));
assert.ok(registry.negativeRows.some((row) => row.id === "duplicate-execution-receipt"));
assert.ok(registry.negativeRows.some((row) => row.id === "stale-execution-receipt"));
assert.ok(registry.negativeRows.some((row) => row.id === "execution-transfer-mismatch"));
