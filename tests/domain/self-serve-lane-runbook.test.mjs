...
assert.ok(runbook.lanes.some((lane) => lane.id === "defi-lab" && lane.openRail.some((rail) => /AMM custody/.test(rail))));
assert.ok(runbook.lanes.some((lane) => lane.id === "user-wallet-handoff" && lane.commands.includes("npm run wallet:standard-requests")));
