...
assert.equal(artifact.currentEvidence.liveSpendPreflight, "accepted-script-enforced-under-cap-spend");
...
assert.ok(artifact.localSourceFindings.some((item) => item.id === "funded-output-live-spend-preflight" && item.status === "accepted-script-enforced-under-cap-spend"));
...
assert.ok(artifact.buildRulesForAgents.some((rule) => /accepted SCRIPT_ENFORCED under-cap spend/.test(rule)));
assert.ok(artifact.nextSteps.some((step) => /continuation output/.test(step.task)));
