...
const [dossier, acceptedOutputs, settlementDrafts, covenantReleaseEvidence, covenantRefundEvidence, checkedIn] = await Promise.all([
...
assert.equal(artifact.status, "accepted-covenant-release-and-refund-spends");
...
assert.equal(artifact.currentEvidence.acceptedCovenantRefundSpends, 3);
assert.equal(artifact.currentEvidence.releaseAndRefundUseSeparateFreshOutputs, true);
...
assert.ok(artifact.blockers.some((item) => /separate fresh/.test(item)));
