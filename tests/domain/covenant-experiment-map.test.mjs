...
assert.ok(map.experiments.some((item) => item.id === "treasury-wars" && item.status === "build-first"));
...
assert.ok(map.experiments.some((item) => item.id === "covenant-owned-asset-game" && /ICC/.test(item.covenantPattern)));
