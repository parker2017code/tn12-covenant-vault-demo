...
assert.equal(map.summary.scriptEnforcedClaims, 1);
assert.deepEqual(map.spotlight, ["blitz-mux-arena", "treasury-wars", "covenant-owned-asset-game"]);
assert.ok(map.experiments.some((item) => item.id === "treasury-wars" && item.status === "accepted-script-enforced-under-cap-spend"));
