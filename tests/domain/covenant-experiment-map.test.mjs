...
assert.equal(map.summary.acceptedGenesisPreflights, 2);
...
assert.ok(map.experiments.some((item) => item.id === "blitz-mux-arena" && item.status === "accepted-genesis-funded-preflight" && item.currentRepoEvidence.includes("fixtures/BlitzMuxArenaContractOutpoint.json")));
