...
The repo is a TN12 covenant/app primitive workshop. It has accepted TN12 proof transactions for vault recovery, vault delayed withdrawal, individual assurance release, individual assurance refund, escrow release, escrow DAA-score refund, and escrow mutual cancel. It also has 18 accepted TN12 payload events for invoice, access-pass, auction, stable-value issuer, miner/watcher attestation, and agent commitment state.
...
- The payload vertical slice has 18 accepted TN12 JSON wRPC events listed in `fixtures/PayloadEventEvidence.json`. Keep app state tied to matched accepted payload bytes.
...
The latest continuation replaced fake accepted payload fixture txids with real TN12 evidence and expanded payload verification to 18 accepted events:
...
- access-pass redemption, auction bids, auction settlement/refund planner events, stable-value issuer issuance/redemption, miner/watcher attestation, and agent task/proof/dispute events are listed in `fixtures/PayloadEventEvidence.json`.

`docs/TN12_TEST_MATRIX.md` now tracks what is TN12 accepted, what is only local reducer-tested, and what still needs safe TN12 transactions. `fixtures/SubmitConsoleDrafts.json` now exposes 31 reviewable drafts, including all 18 accepted payload drafts. `artifacts/checkpointed-accepted-index.json` indexes 25 public TN12 reads: 7 proof spends plus 18 payload events.
...
Observed proof state remained accepted and matched: `7/7` accepted proof transactions, `7/7` P2SH inputs, `7/7` P2PK outputs. Payload events verified: `18/18`. Checkpointed index matched: `25/25`.
