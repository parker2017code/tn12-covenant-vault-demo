...
- The payload vertical slice has 20 accepted TN12 JSON wRPC events listed in `fixtures/PayloadEventEvidence.json`. Keep app state tied to matched accepted payload bytes.
...
The latest continuation replaced fake accepted payload fixture txids with real TN12 evidence and expanded payload verification to 20 accepted events:
...
Observed proof state remained accepted and matched: `7/7` accepted proof transactions, `7/7` P2SH inputs, `7/7` P2PK outputs. Payload events verified: `20/20`. Checkpointed index matched: `27/27`, and the persisted checkpoint guard reported no rollback.
