export function buildCovenantExperimentMap({ generatedAt = new Date().toISOString() } = {}) {
  const experiments = [
    ...
    experiment({
      ...
      nextBuildSteps: [
        "Fund a fresh covenant-bound pledge set instead of reusing the spent P2PK pledge outputs.",
        "Prove release/refund locally for each fresh covenant pledge output.",
        "Submit accepted TN12 covenant release/refund evidence before upgrading the status."
      ],
      hardBoundary: "This is transparent coordination evidence with an accepted TN12 release and a covenant-settlement target artifact. The existing pledge outputs are P2PK and already spent, so fresh covenant pledge outputs are required before calling this covenant-settled."
    }),
    experiment({
