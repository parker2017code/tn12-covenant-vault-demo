...
- `docs.kas.fyi` adds a hosted API-key option for second-source checks: `/v1/transactions/acceptance` returns acceptance status, accepting block hash, and confirmations for batches of txids; block range endpoints can query by blue score or DAA score; transaction-output endpoints can inspect spend state. Treat this as hosted infrastructure, not replacement evidence for the repo's accepted TN12 artifacts or future production indexer.
- KDP data-type notes matter for app code: KAS amounts should stay in sompi strings until explicitly converted, because decimal JS numbers can lose precision. One KAS equals `100000000` sompi.
