...
The single ordered project queue is generated by `npm run project:queue` into `artifacts/next-work-queue.json`. It is the current answer for what is done, WIP, roadmap, and next 30 tasks. The stop condition for this phase is written in `docs/PROJECT_COMPLETION_PLAN.md`; use it before adding another fixture-only app lane.
