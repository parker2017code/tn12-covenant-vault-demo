...
| 4 | Split `app.js` by proof page, lab page, renderers, and data loaders. | UI changes should not affect proof verification or unrelated lab panels. Shared formatting/form/data-loading helpers are now extracted; fixture/artifact fetches now use the shared loader, and the DeFi simulation reviewer surface is now in `src/ui/renderers/defiSimulationSurface.mjs`. More renderer/page extraction remains next. | No |
