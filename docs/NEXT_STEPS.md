...
- Proof-lab auditability is roughly `83-86%` after the focused-check split, address validation pass, proof-record mutation coverage, status-artifact checks, wallet-submit readiness checks, attestation/invoice/research checks, batch-assurance checks, and escrow-marketplace checks.
...
|---|---|---|---|
| 1 | Keep splitting `scripts/check.mjs` into focused domain tests. | Smaller failures are easier for an outside reviewer to trust and debug. Wallet-submit, attestation/invoice/research, batch-assurance, and escrow-marketplace slices now have focused tests. | No |
