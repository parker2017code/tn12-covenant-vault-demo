...
- Scheduler evidence now has an accepted covenant payout spend in `artifacts/scheduler-covenant-payout-evidence.json` and local payout rejects in `artifacts/scheduler-covenant-payout-negative-evidence.json`; trigger eligibility, winning-bid selection, and stale/duplicate blocking remain replay/indexer-derived.
