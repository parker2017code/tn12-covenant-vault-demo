...
| Scheduler trigger intents can be registered and reduced from accepted TN12 payloads. | `TN12_ACCEPTED` | `INDEXER_DERIVED`; the accepted payload registers an intent and the reducer checks eligibility. Execution remains local-key review, not protocol scheduling. | `artifacts/scheduler-intent-registry.json`, `artifacts/payload-scheduler-intent-pool-rebalance-001-evidence.json`, `docs/TANGVM_UNISC_BOUNDARY.md` | `npm run scheduler:intents` |
