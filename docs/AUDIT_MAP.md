...
| AMM, lending, liquidation, oracle, bridge, and full DeFi are live. | `PLANNER_ONLY` | `PLANNER_ONLY` / research. Simulation can exercise AMM, lending, liquidation, oracle, and settlement rules against accepted receipt/indexer inputs; the reducer can promote review-only state while blocking custody promotion; advanced hardening covers LP shares, invariants, health sweeps, and oracle failure cases; the multi-wallet pack groups roles without external-signer claims; the manifest checks schemas, zero live-product claims, zero custody actions, zero external-signer claims, and no secret-like fields. It does not prove custody or live DeFi. | `artifacts/defi-artifact-manifest.json`, `artifacts/defi-planner-simulation.json`, `artifacts/defi-scenario-simulation.json`, `artifacts/defi-scenario-reducer.json`, `artifacts/defi-advanced-simulation.json`, `artifacts/defi-multi-wallet-scenario-pack.json`, `artifacts/defi-backlog.json`, `artifacts/missing-rails-matrix.json`, `artifacts/oracle-source-matrix.json` | `npm run defi:manifest` |
