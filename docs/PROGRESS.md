...
- Live wallet submit path that preserves payload bytes and exact tx fields without local private keys. The wallet-submit handoff package, connector request bundle, dry-run adapter review sessions, submit-result ledger, result validator, external-signer gap artifact, and KasSigner/KasSee reference boundary are built; a real wallet adapter still needs unsigned or partial transaction requests before it can sign/submit externally.
