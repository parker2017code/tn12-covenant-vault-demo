...
- Batch-assurance pledge funding draft: `npm run campaign:pledge-funding-draft` writes `artifacts/signed-drafts/batch-assurance-pledge-funding.json` plus public pledge-wallet metadata. It signs the exact 45/35/20 TKAS output plan but does not broadcast or mark custody ready.
