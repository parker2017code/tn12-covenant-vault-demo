...
When this repo uses or reports a completion percentage, read it as mainnet deployment readiness unless a line explicitly says it is only describing TN12 proof-core progress.

Use `docs/TN12_ENDPOINTS.md` for the verified public TN12 endpoint and `docs/PROOF_INDEX.md` for the compact proof index. The portable endpoint sample now lives in `.env.tn12.example`.
...
The current 10-task execution slice is generated by `npm run project:next-ten-status` and written to `artifacts/next-ten-execution-status.json`. It records 6/10 locally completed tasks, 18 percentage points of local-slice progress, a 50-53% local completion estimate for this DeFi/TN12 demo lane, and keeps real external wallet signing as the next major blocker.
