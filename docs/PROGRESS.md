...
- 39 payload events accepted on TN12, including seven DeFi v1 receipts, one scheduler-intent receipt, three scheduler-bid receipts, one scheduler-execution receipt, and one agent release wallet-review event.
...
- One scheduler-intent payload, three scheduler-bid receipts, one scheduler-execution receipt, and one local-key payout are accepted on TN12 and reduced as executed trigger state.
